...
class Court:
    ...
    def __init__(self, thisLocator, locatorID, courtType, location, pricePerHour, maxPlayers, courtagenda):
        ...
        if userexsits:
            ...
            __class__.courtData[self.locatorID] = {
                # VERIFICAR SE ESTÁ FUNCIONANDO ( )
                self.courtID: self.__dict__}
            thisLocator.ownedCourts.append(self)
    ...
    @staticmethod
    def getCourtName(courtID, locatorID):
        courtName = __class__.courtData[locatorID][courtID]["courtType"]

        return courtName

    def updateCourtData(self, userexsits):
        # create csv for each locator
        newCourtData = pd.DataFrame(
            self.__class__.courtData[self.locatorID][self.courtID], index=[self.courtID])
        if userexsits:
            oldCourtData = pd.read_csv(
                f"courtData/locator{self.locatorID}CourtData.csv")
            newCourtData = pd.concat(
                [oldCourtData, newCourtData], ignore_index=True)
        newCourtData.to_csv(
            f"courtData/locator{self.locatorID}CourtData.csv", index=False)
    ...
    def cancelBooking(self, resID):
        ...
        if self.checkAvailability(agenda.Agenda.agendaData[self.courtID][reservationToCancel[1][0]][reservationToCancel[1][1]:reservationToCancel[1][2]]) == False:
            agenda.Agenda.updateAgenda(
                resID, reservationToCancel[1][0], reservationToCancel[1][1], reservationToCancel[1][2], [None, True])
